...
func (s *DeviceState) prepareDevices(claim *resourceapi.ResourceClaim) (PreparedDevices, error) {
	...
	// Add the default CPU Config to the front of the config list with the
	...
	configs = slices.Insert(configs, 0, &OpaqueDeviceConfig{
		Requests: []string{},
		Config:   configapi.DefaultCpuConfig(),
	})
	...
	for _, result := range claim.Status.Allocation.Devices.Results {
		if _, exists := s.allocatable[result.Device]; !exists {
			return nil, fmt.Errorf("requested CPU is not allocatable: %v", result.Device)
		}
	...
	for c, results := range configResultsMap {
		// Cast the opaque config to a CpuConfig
		var config *configapi.CpuConfig
		switch castConfig := c.(type) {
		case *configapi.CpuConfig:
			config = castConfig
		default:
		...
		if err := config.Normalize(); err != nil {
			return nil, fmt.Errorf("error normalizing CPU config: %w", err)
		}

		// Validate the config to ensure its integrity.
		if err := config.Validate(); err != nil {
			return nil, fmt.Errorf("error validating CPU config: %w", err)
		}

		// Apply the config to the list of results associated with it.
		containerEdits, err := s.applyConfig(config, results)
		if err != nil {
			return nil, fmt.Errorf("error applying CPU config: %w", err)
		}
...
func (s *DeviceState) applyConfig(config *configapi.CpuConfig, results []*resourceapi.DeviceRequestAllocationResult) (PerDeviceCDIContainerEdits, error) {
	perDeviceEdits := make(PerDeviceCDIContainerEdits)

	for _, result := range results {
		envs := []string{
			fmt.Sprintf("CPU_DEVICE_%s=%s", result.Device[4:], result.Device),
		}

		if config.Sharing != nil {
			envs = append(envs, fmt.Sprintf("CPU_DEVICE_%s_SHARING_STRATEGY=%s", result.Device[4:], config.Sharing.Strategy))
		}

		switch {
		case config.Sharing.IsRealTime():
			rtconfig, err := config.Sharing.GetRealTimeConfig()
			if err != nil {
				return nil, fmt.Errorf("unable to get realtime config for device %v: %w", result.Device, err)
			}
			envs = append(envs,
				fmt.Sprintf("CPU_DEVICE_%s_RT_RUNTIME_US=%v", result.Device[4:], rtconfig.RuntimeUs),
				fmt.Sprintf("CPU_DEVICE_%s_RT_PERIOD_US=%v", result.Device[4:], rtconfig.PeriodUs),
			)
		case config.Sharing.IsCfs():
			cfsconfig, err := config.Sharing.GetCfsConfig()
			if err != nil {
				return nil, fmt.Errorf("unable to get CFS config for device %v: %w", result.Device, err)
			}
			envs = append(envs, fmt.Sprintf("CPU_DEVICE_%s_CFS_SHARES=%v", result.Device[4:], cfsconfig.Shares))
		}
